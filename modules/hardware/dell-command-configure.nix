...
let
  ...
  configFile = pkgs.writeTextFile {
    ...
    text = ''
      ...
      hapi.configtool=${pkgs.maxine.dell-command-configure}/bin/dchcfg
      hapi.installpath=${pkgs.maxine.dell-command-configure}
...

{
  ...
  config = mkIf config.hardware.dellCommandConfigure.enable {
    environment.systemPackages = [ pkgs.maxine.dell-command-configure ];
